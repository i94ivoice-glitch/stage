...
| 📝 字幕字體大小 | FontSize **38** |
| 📝 字幕最大寬度 | 58 視覺單位（約 29 中文字，最多 3 行）|
| 📝 字幕標點 | 移除 `，。；：、！…—` / 保留 `？` |
| 📝 字幕位置 | Alignment=2（底部置中），MarginV=40，MarginL/R=60 |
| 📝 字幕樣式 | 白字 Bold，黃邊框 Outline=3 |
...
# zoom = 1.0 + 0.2 × (1 - |2×(t mod 46)/46 - 1|)
KB_FILTER="scale=w='1280*(1+0.2*(1-abs(2*mod(t,46)/46-1)))':h='720*(1+0.2*(1-abs(2*mod(t,46)/46-1)))':eval=frame,crop=1280:720,format=yuv420p"
...
t= 23s → 1.2× (最大)
t=34.5s → 1.1×
t= 46s → 1.0× (回到起點，開始下一循環)
```

長章節（>46s）會自動循環；短章節（<46s）走部分循環亦可。
...
KB_VF = (
    "scale=w='1280*(1+0.2*(1-abs(2*mod(t,46)/46-1)))':"
    "h='720*(1+0.2*(1-abs(2*mod(t,46)/46-1)))':"
    "eval=frame,"
    "crop=1280:720,"
    "format=yuv420p,"
    "fps=24"
